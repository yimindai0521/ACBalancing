...
#' @description Hierarchically Regularized Entropy Balancing expands the feature space
#' by including higher-order terms (including interactions) of covariates
#' and then achieves approximate balance on the expanded features.
#' effect.
#'
#' @details
...
#' @export
#'
HRB <- function(x, treat, group1, group2 = NA, outcome, iterations = 1000) {

  x <- as.matrix(x)
  treat <- as.factor(treat)
  group1 <- as.factor(group1)
  if(!is.null(group2)){ group2 <- as.factor(group2) }
